<script>
    ...
    // Нужно, чтобы изображения товара менялись после обновления
    onMount(() => {
        // Помечаем, что изображения товара на слайдере создания сета пагинируются
        initialSliders();
    });
    ...
    //     // if ($render_in_progress || ($render_in_progress && !$got_result_images)) {
    //     //     curr_style = "js--hidden"; // анимация рендеринга
    //     // } else if (
    //     //     $product_images.length > 0 ||
    //     //     $got_result_images
    //     // ) {
    //     //     curr_style = "js--none"; // есть изображения, которые нужно вывести
    //     // } else {
    //     //     curr_style = "js--active"; // окно, когда выводить на слайдер нечего
    //     // }
    ...
    function getImagesSliderStyle() {
        ...
        if ($show_product_images || $render_task_results.images.length > 0) {
            curr_style = "js--none";
            ...
            if (
                $render_task_results.images.length > 0 &&
                ...
                curr_style = "js--active"; // Here will be photos
            }
...
<div class="catalogs__set-free-img-wrapper">
    {#if $show_product_images}
        <div
            class="catalogs__set-free-img swiper"
            style={$render_in_progress ? "js--hidden" : ""}
    ...
    {:else}
        <div class="catalogs__set-free-img-wrapper">
            <div class="catalogs__set-free-img swiper" data-name="slider-set">
                <div class="swiper-wrapper">
                    {#each $render_task_results.images as image}
                        <img class="swiper-slide" src={image} alt="image" />
                    {/each}
                </div>
                <div
                ...
                >
                    <span
                        ...
                        aria-current="true"
                    ></span>
                    {#each $render_task_results.images as _}
                        <span class="swiper-pagination-bullet"></span>
                    {/each}
                </div>
                <span
