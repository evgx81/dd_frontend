<script>
    ...
    import { is_search_results_page } from "./stores";

    /**
     * Хранит значение поискового запроса из URL-параметров 
     * @type {string}
     */
    let search_query_value = "";
    afterUpdate(() => {
        // Если перешли на страницу результатов глобального поиска сетов товаров,
        // то получаем значение поискового запроса из URL-параметров
        if ($is_search_results_page) {
            const urlParams = new URLSearchParams(window.location.search);
            search_query_value = urlParams.get("q");
        }
    });
</script>
